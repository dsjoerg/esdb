...
for second in np.arange(5,21) * 60:
	...
	fignum=1
        for league in range(0,6):
                ...
                plt.hexbin(snapnow['army1'], snapnow['income1'], snapnow['winner'], cmap=plt.cm.YlOrRd_r, gridsize=30, extent=(0,xmax,0,ymax))
                plt.title(leaguenames[league])

        plt.savefig("{}/aiwin_{}.pdf".format(resdir, second))
